...
class OVSAM(BaseModel):
    ...
    def predict(self, batch_inputs: Tensor, batch_data_samples: SampleList) -> Union[Dict, List]:
        backbone_feat = self.backbone(batch_inputs)
        batch_feats = self.neck(backbone_feat)
        fpn_feats = self.fpn_neck(backbone_feat) if self.fpn_neck else None
        #----------------------------------
        import json 
        with open('features_4layer.json', 'r') as f:
            loaded_data = json.load(f)
        updated_features_tensor = torch.tensor(loaded_data)
        GCN_Feature = updated_features_tensor
        # Define a background token (zero vector)
        back_token = torch.zeros(1, GCN_Feature.size(1), dtype=torch.float32, device=GCN_Feature.device)
        # Concatenate the background token with GCN_Features
        GCN_Feature = torch.cat([GCN_Feature, back_token], dim=0)  # Now it becomes [14, 768]
        #---------------------------------------------------------------------------------------------
        for feat, data_sample in zip(batch_feats, batch_data_samples):
            ...
            sparse_embed, dense_embed = self.pe(
                ...
                # with_bboxes=not self.use_point,
                # with_points=self.use_point
                with_bboxes=False,
                with_points=False,
            )

            kwargs = {
                'backbone_feats': backbone_feat if self.enable_backbone else None,
                'backbone': self.backbone if self.enable_backbone else None
            }
            if fpn_feats is not None:
                masks, _, cls_pred = self.mask_decoder(
                ...
                masks, _, cls_pred = self.mask_decoder(
                    ...
                    dense_prompt_embeddings=dense_embed,
                    multi_mask_output=True,
                    GCN_Feature=GCN_Feature,  # 添加GCN特征
                    data_samples=data_sample,
                    backbone=self.backbone,  # 传递 self.backbone
                    neck=self.neck,  # 传递 self.neck
                    # **kwargs
                )
            ...
            masks = postprocess_masks(
                ...
                original_size=data_sample.img_shape,
            )
            masks = masks.sigmoid() > self.MASK_THRESHOLD  # [N, C, H, W]

            cls_scores =  cls_pred.mean(dim=1)
            max_confidence_indices = torch.argmax(cls_scores[..., :-1], dim=-1) 
            selected_masks = torch.zeros_like(masks[:, 0])  # [N, H, W]
            for i, idx in enumerate(max_confidence_indices):
                selected_masks[i] = masks[i, idx]
            results = InstanceData()
            results.masks = selected_masks  # [N, H, W]
            results.labels = max_confidence_indices  # [N]

            if not self.use_gt_prompt:
                scale_factor = data_sample.pred_instances.bboxes.new_tensor(
                    data_sample.scale_factor).repeat(2)
