...
#代表支持的所有模型，也定义成字典的形式
default_cfgs = {
    # patch models，16代表patch的大小，224就代表输入图片的大小
...
class TransReID(nn.Module):
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'decoder'in k:
                continue    
            if 'pos_embed_probs'in k:
                continue    
            if 'get_token_probs'in k:
                continue  
            if "decode_text" in k:
                continue
            if "proj_encode_feature" in k:
                continue
...
def vit_large_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
    model = TransReID(
