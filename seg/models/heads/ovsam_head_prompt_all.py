...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None):
        ...
        前向传播过程
        Args:
            query: [Batch, QueryLen, EmbedDim]
            key: [Batch, KeyLen, EmbedDim] 
            value: [Batch, ValueLen, EmbedDim]
            key_padding_mask: [Batch, KeyLen] (可选)
        Returns:
            attn_output: [Batch, QueryLen, EmbedDim]
            attn_weights: [Batch, NumHeads, QueryLen, KeyLen]
        """
...
class OVSAMHead(BaseModule):
    ...
    def __init__(
        ...
        self.cdt_t = ContentDependentTransfer(d_model=256, nhead=8)#【修改】
        self.cdt_c = ContentDependentTransfer(d_model=256, nhead=8)#【修改】
        ...
        self.PPEM = Prototype_Prompt_Encoder_MultiClass(
            feat_dim=256,
            hidden_dim_dense=128,
            hidden_dim_sparse=128,
            size=64,
    ...
    def predict_masks(
        ...
        prototypes = GCN_Features[:-1].to(image_embeddings.device)
        prototypes = self.prototype_projection(prototypes) 
        ...
        # 修改这里：增加引用图片数量，可以配置为3或5
        num_examples = getattr(self, 'num_reference_examples', 5) 
        ...
        if not hasattr(self, 'reid_model') or self.reid_model is None:
            CONFIG_FILE = (
                "/data1/Code/wangziwen/Project/123/ovsam-main/seg/models/TransReid/"
                "configs/VeRi/vit_transreid_stride_new.yml"
            )
            WEIGHTS_PATH = (
                "/data1/Code/wuwentao/pre_model/"
                "0921_vehiclemaev2_100W_mask75/checkpoint-100.pth"
            )
            # GALLERY_DIR = "/data1/Datasets/Seg/CarPart/images/valid/"
            GALLERY_DIR = "/data1/Datasets/Seg/wzw/images/train/"
        ...
        expanded_prototypes = self.cdt_t(image_embeddings, expanded_prototypes)
        class_prototypes = self.cdt_c(image_embeddings, class_prototypes)
        B, C, H, W = image_embeddings.shape
        feat = image_embeddings.permute(0, 2, 3, 1).reshape(B, H * W, C)
        prototypes = expanded_prototypes.to(feat.device)
        dense_emb_all, sparse_emb_all = self.PPEM(feat, prototypes)
        ...
        queries = torch.cat((output_tokens, sparse_emb_all), dim=1)

        # Process image embeddings
        image_embeddings = image_embeddings + dense_emb_all
        ...
        if self.with_label_token:
            ...
            cls_embed = torch.stack(cls_embed_list, dim=1)  # Shape: [B, 13, 768]

            if self.gen_box:
                # Modified code starts here
                # Get masks from first batch since all instances share the same masks
