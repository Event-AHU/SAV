...
param_scheduler = [
    dict(
        type=LinearLR,
        start_factor=0.01,  # Increased from 0.001 to 0.01 for a gentler warmup
        by_epoch=False,
        begin=0,
        end=200  # Shortened from 500 to 200 iterations
    ),
    dict(
        type=MultiStepLR,
        begin=0,
        ...
        by_epoch=True,
        milestones=[6, 10],  # Adjusted from [8, 11] to [6, 10]
        gamma=0.1
    )
]
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        type=AdamW,
        lr=0.00005,  # Changed from 0.0001 to 0.00005
        weight_decay=0.01,
        eps=1e-8,
        betas=(0.9, 0.999)
    ),
